...
#include "random.h"
  ...
  // Define crypto_ops class to avoid including crypto.h
  class crypto_ops {
    crypto_ops();
  ...
  // COMMENTED OUT - Not needed for generate_key_derivation / derive_public_key
  /*
  ...
  */
  ...
  // Wrapper functions to expose crypto_ops members to the crypto namespace
  ...
  void hash_to_ec(const PublicKey &key, KeyImage &res) {
    ...
    ge_p2 point2;
    ge_p1p1 point_tmp;
    cn_fast_hash(std::addressof(key), sizeof(PublicKey), reinterpret_cast<char *>(&h));
    ge_fromfe_frombytes_vartime(&point2, reinterpret_cast<const unsigned char *>(&h));
    ge_mul8(&point_tmp, &point2);
    ge_p1p1_to_p3(&point, &point_tmp);
